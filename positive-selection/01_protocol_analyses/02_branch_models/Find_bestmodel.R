...
lnL_vals <- read.table( file = "lnL_branch_mods.txt", sep= " ", stringsAsFactors = FALSE, 
                        header = FALSE )
row.names( lnL_vals ) <- c( "Chicken-branch", "Duck-branch", "DuckChicken-branch", "BirdClade-branch", "M0-nobranch" )
...
# p-val = 9.076912-13 < 0.05
Chisq.crit.duckchick <- qchisq( p = 0.95, df = 1 )
# alpha critical value = 3.841459

## 2.4. Bird-branch vs M0-nobranch ##
diff_birdVSM0nob <- 2*( lnL_vals[4,] - lnL_vals[5,] )
# diff = 51.03414
pchisq( diff_birdVSM0nob, df = 2, lower.tail=F )
# p-val = 8.280896e-12 < 0.05
Chisq.crit.bird <- qchisq( p = 0.95, df = 2 )
...
text( x = coords_alphac[1], y = coords_alphac[2],
      ...
      cex = 1.2, col = "gray" )
text( x = coords_pval[1], y = coords_pval[2],
      labels = expression( atop( ''*italic( pval )*' = 9.08-13' ) ),
      cex = 1.2, col = "black" )
...
text( x = coords_alphac[1], y = coords_alphac[2],
      ...
      cex = 1.2, col = "gray" )
text( x = coords_pval[1], y = coords_pval[2],
      labels = expression( atop( ''*italic( pval )*' = 8.28e-12' ) ),
      cex = 1.2, col = "black" )
title( expression( 'D) '*italic(Bird)*': Branch model VS M0 model ' ) )
